...
public enum PublicHolidays {
    ...
    public static func all(countryCode: String, subTerritoryCode: String? = nil, timeZone: TimeZone = Calendar.current.timeZone) throws -> [PublicHoliday] {
    ...
    public static func contains(
        ...
        guard supportedYears.contains(date.year(timeZone: timeZone)) else { throw PublicHolidaysError.dateOutsideOfSupportedRange }
        let publicHolidaysAtLocation = try all(countryCode: countryCode, subTerritoryCode: subTerritoryCode, timeZone: timeZone)

        guard let firstPublicHolidayEndingAfterDate = publicHolidaysAtLocation.first(where: { $0.date.addingTimeInterval(.days(1)) > date }) else {
            return false
        }
